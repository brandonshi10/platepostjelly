...
| HEAD commit at the time this document was written | `098b4f989cfb95769940e884067972382576104` |
...
This is **not** an authentication or deployment blocker — the local
anonymous deployment pushes and runs these files successfully with
`--typecheck disable` (they are valid, bundleable JavaScript; only the
static types are wrong for the retired table names). It is a
type-correctness gap in code these three files' errors are entirely
resolved by the Plan 2 rewrite that already owns them, not by regenerating
bindings. Re-run `pnpm typecheck:convex` after Plan 2 lands
`convex/jellyhunt/submissions.ts` (or an equivalent namespaced replacement)
and retires `convex/audit.ts`, `convex/missions.ts`, and
`convex/submissions.ts`, and this gate should go green without further
schema changes.
